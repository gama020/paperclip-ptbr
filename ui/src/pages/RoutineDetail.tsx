...
const concurrencyPolicyDescriptions: Record<string, string> = {
  coalesce_if_active: "routineDetail.concurrencyCoalesceIfActive",
  always_enqueue: "routineDetail.concurrencyAlwaysEnqueue",
  skip_if_active: "routineDetail.concurrencySkipIfActive",
};
const catchUpPolicyDescriptions: Record<string, string> = {
  skip_missed: "routineDetail.catchUpSkipMissed",
  enqueue_missed_with_cap: "routineDetail.catchUpEnqueueMissedWithCap",
};
const signingModeDescriptions: Record<string, string> = {
  bearer: "routineDetail.signingModeBearer",
  hmac_sha256: "routineDetail.signingModeHmacSha256",
  github_hmac: "routineDetail.signingModeGithubHmac",
  none: "routineDetail.signingModeNone",
};
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">
          {trigger.kind === "schedule" && trigger.nextRunAt
            ? t('routineDetail.nextRun', { nextRun: new Date(trigger.nextRunAt).toLocaleString() })
...
export function RoutineDetail() {
  ...
  const saveRoutine = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t('routineDetail.couldNotSaveRoutine'),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t('routineDetail.couldNotStartRoutineRun'),
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t('routineDetail.failedToUpdateRoutine'),
        body: error instanceof Error ? error.message : t('routineDetail.couldNotUpdateRoutine'),
        tone: "error",
      });
    },
  });
  ...
  const createTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      if (result.secretMaterial) {
        ...
        pushToast({
          title: t('routineDetail.triggerAdded'),
          body: t('routineDetail.routineScheduleSaved'),
          tone: "success",
        });
      }
      await Promise.all([
  ...
  const updateTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({
        title: t('routineDetail.triggerSaved'),
        body: t('routineDetail.routineCadenceSaved'),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
  ...
  const deleteTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({
        title: t('routineDetail.triggerDeleted'),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
  ...
  const rotateTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t('routineDetail.failedToRotateWebhookSecret'),
        body: error instanceof Error ? error.message : t('routineDetail.couldNotRotateWebhookSecret'),
        tone: "error",
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message={t('routineDetail.selectCompanyToViewRoutines')} />;
  }
            ...
            onCheckedChange={() => {
              if (!automationEnabled && !routine.assigneeAgentId) {
                pushToast({
                  ...
                  body: t('routineDetail.setDefaultAgentFirst'),
                  tone: "warn",
                });
                return;
              }
...
<TabsTrigger value="activity" className="gap-1.5">
        ...
        <TabsContent value="triggers" className="space-y-4">
          ...
          {routine.triggers.length === 0 ? (
            <p className="text-xs text-muted-foreground">{t('routineDetail.noTriggersConfigured')}</p>
          ) : (
