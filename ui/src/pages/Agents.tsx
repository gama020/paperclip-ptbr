...
export function Agents() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("agent.agents") }]);
  }, [setBreadcrumbs, t]);

  if (!selectedCompanyId) {
    return <EmptyState icon={Bot} message={t("agent.select_company_to_view_agents")} />;
  }
  ...
  return (
    <div className="space-y-4">
        ...
        <Tabs value={tab} onValueChange={(v) => navigate(`/agents/${v}`)}>
          <PageTabBar
            items={[
              ...
              { value: "paused", label: t("run.paused") },
      ...
      {filtered.length > 0 && (
        <p className="text-xs text-muted-foreground">{filtered.length} {filtered.length !== 1 ? t("agent.agents").toLowerCase() : t("agent.title").toLowerCase()}</p>
      )}
      ...
      {agents && agents.length === 0 && (
        <EmptyState
          icon={Bot}
          message={t("agent.create_first_agent")}
          action={t("agent.new_agent")}
      ...
      {effectiveView === "list" && agents && agents.length > 0 && filtered.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-8">
          {t("agent.no_agents_match_filter")}
        </p>
      )}
  ...
  return (
    ...
    >
      ...
      <span className="text-[11px] font-medium text-blue-600 dark:text-blue-400">
        {t("dashboard.live")}{liveCount > 1 ? ` (${liveCount})` : ""}
      </span>
    </Link>
  );
}
